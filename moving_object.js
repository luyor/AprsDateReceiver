...
var otherstream=fs.createWriteStream('other.log',{flag:'a'});
var exceptionstream=fs.createWriteStream('exception.log',{flag:'a'});
...
conn.on('data',function(data){
    ...
    for(i=0;i<array.length-1;i++) {
        ...
        if(header&&info) {
            object["Source"] = header.split('>')[0];
            if (header.split('>')[1]) object["Destination"] = header.split('>')[1].split(',')[0];
            if (info[0] == '!' || info[0] == '=') { //without Timestamp
                ...
                {
                    setSymbol(com_obj, symbol, object["Destination"]);
                    //console.log(com_obj);
